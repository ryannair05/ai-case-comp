...
"""
        let result = try await call(system: system, user: "Generate a proposal for this RFP:\n\n\(rfpText)")
    ...
    func generateSupportResponse(ticketBody: String, kbContext: [VectorMatch], tier: String) async throws -> String {
    ...
    func extractMeetingSignals(customerId: String, notes: String, clientName: String) async throws -> [String: Any] {
        ...
        let raw = try await call(
            system: "You are a sales intelligence AI. Extract structured signals from meeting notes. "
    ...
    func generateOutreachSequence(senderFirm: String, prospect: [String: Any], winContext: [VectorMatch], sequenceLength: Int = 4) async throws -> [[String: Any]] {
    ...
    private func call(system: String?, user: String, maxTokens: Int = 4096) async throws -> String {
        ...
        guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
            ...
            throw ClaudeError.apiError("HTTP \((response as? HTTPURLResponse)?.statusCode ?? 0): \(body)")
        }
